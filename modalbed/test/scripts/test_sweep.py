...
from modalbed.scripts import sweep
...
class TestSweep(unittest.TestCase):
    ...
    def test_job(self):
        ...
        self.assertEqual(job.command_str,
            f'python -m modalbed.scripts.train --foo bar --output_dir {job.output_dir}')
    ...
    def test_end_to_end(self):
        ...
        result = subprocess.run(f'python -m modalbed.scripts.sweep launch '
        ...
        # get relaunched.
        subprocess.run(f'python -m modalbed.scripts.sweep delete_incomplete '
            f'--data_dir={os.environ["DATA_DIR"]} --output_dir={output_dir} '
            f'--algorithms ERM --datasets Debug28 --n_hparams 1 --n_trials 1 '
            f'--command_launcher dummy --skip_confirmation',
            shell=True, capture_output=True)
