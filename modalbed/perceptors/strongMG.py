...
from modal_encoder.model import data
...
modalityLoader = make_modalityLoader(
    {
        ModalityType.TEXT: lambda x, device: text_preprocessor(
            x, padding=True, truncation=True, return_tensors="pt"
        ).input_ids.to(device),  # data.load_and_transform_text,
        ModalityType.VISION: data.load_and_transform_vision_data,
        ModalityType.AUDIO: lambda x, device: data.load_and_transform_audio_data(
            x, device, num_mel_bins=224, target_length=224
        ).mean(dim=2),  # [bs, 3 , 1, H, W]
        ModalityType.VIDEO: lambda x, device: data.load_and_transform_video_data(
            x, device
        )
        .mean(dim=1)
        .mean(dim=2),  # [bs, N_clips*3 , 3, N_frame, H, W]
        ModalityType.DEPTH: partial(load_and_transform_depth_data, repeat=True),
    }
)
...
class ContrastivePreceptor(nn.Module):
    ...
    def __init__(
        self, dataset="msrvtt", freeze=True, modal=ModalityType.VISION, train=False
    ):
        ...
        if modal == ModalityType.TEXT:
            ...
            self.model = lambda x: self.encoder_model(input_ids=x).last_hidden_state[
                :, 0, :
            ]
            ...
            self.model = lambda x: self.encoder_model(pixel_values=x).last_hidden_state[
                :, 0, :
            ]
    ...
    def train_forward(self, x):
        ...
        data_1 = modalityLoader(self.modal_name)(
            datas, device
        )  # use the data loader from imagebind
    ...
    def forward(self, x):
        ...
        inputs = modalityLoader(self.modal_name)(datas, device).to(
            device
        )  # use the data loader from imagebind
...
class StrongMGPreceptor(Preceptor):
    def __init__(self, dataset="MSR_VTT", freeze=True, feature_retrieval=False):
        ...
        checkpoint_path_yaml = pkg_resources.resource_filename(
            __name__, "checkpoint_path.yaml"
        )
        ...
        if feature_retrieval:
            ...
            for modal in data:
                ...
                self.models[modal] = ContrastivePreceptor(
                    dataset, freeze=False, modal=modal, train=False
                )
                ...
                self.models[modal] = PeftModel.from_pretrained(
                    self.models[modal],
                    ckpt_path,
                    config=lora_config,
                    is_trainable=False,
