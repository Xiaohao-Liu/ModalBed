...
from modal_encoder.model import load_model, get_embed_dim, LanguageBind
from .base import FeatureStorage, Preceptor
import torch
from modal_encoder.languagebind.languagebind import (
    to_device,
    LanguageBindImageTokenizer,
)
from .utils import make_modalityMap
    ...
    def __init__(self, dataset="msrvtt", freeze=True, feature_retrieval=False):
        ...
        if feature_retrieval:
            ...
            self.modality_transform = {
                c: LanguageBind.transform_dict[c](self.modality_config[c])
                for c in clip_types
            }
    ...
    def forward(self, x):
        ...
        for m_type in datas:
            ...
            if m_type == ModalityType.TEXT:
                inputs[modalityMap(m_type)] = to_device(
                    self.tokenizer(
                        datas[m_type],
                        max_length=77,
                        padding="max_length",
            ...
            #     inputs[modalityMap(m_type)] = to_device(modalityLoader[modalityMap(m_type)](datas[m_type]), device)
                ...
                inputs[modalityMap(m_type)] = to_device(
                    modalityLoader[modalityMap(m_type)](datas[m_type]), device
                )
