...
def _hparams(perceptor, algorithm, dataset, random_seed):
    """
    ...
    # For multimodal perceptors
    _hparam('perceptor', perceptor, lambda r: perceptor)
    _hparam('dataset', dataset, lambda r: dataset)
    _hparam('freeze_perceptor', True, lambda r: True)
    ...
    if algorithm in ['DANN', 'CDANN']:
        ...
        _hparam('n_meta_test', 1, lambda r:  r.choice([1, 2]))
    ...
    if dataset in SMALL_IMAGES:
        ...
        elif algorithm in ["OGM", "LFM", "Concat"]:
            _hparam('lr', 1e-3, lambda r: 10**r.uniform(-4, -2))
        else:
            if "strongMG" in perceptor and dataset == "NYUDv2":
                _hparam('lr', 1e-3, lambda r: 10**r.uniform(-4, -2)) # NYUDv2 StrongMG Setting
            else:
    ...
    if algorithm in ["LFM", "OGM", "Concat"]:
        _hparam("momentum", 0.9, lambda r: r.uniform(0.85, 0.95))
        _hparam("wc", 1.0e-4, lambda r: 10**r.uniform(-3.5, -4.5))
        _hparam("patience", 70, lambda r: r.uniform(60, 80))
        _hparam('gamma', 0.1, lambda r: r.uniform(0, 0.5))
    
    if algorithm in ["OGM"]:
        _hparam("alpha", 0.1, lambda r: r.uniform(0.1, 0.3))
        
        
    return hparams


def default_hparams(perceptor, algorithm, dataset):
    return {a: b for a, (b, c) in _hparams(perceptor, algorithm, dataset, 0).items()}


def random_hparams(perceptor,algorithm, dataset, seed):
    return {a: c for a, (b, c) in _hparams(perceptor, algorithm, dataset, seed).items()}
