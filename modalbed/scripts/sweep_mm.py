...
from modalbed import command_launchers
...
class Job:
    ...
    def __init__(self, train_args, sweep_output_dir):
        ...
        command = ['python', '-m', 'modalbed.scripts.train_mm']
    ...
    def store_command(self, filename):
...
def make_args_list(n_trials, dataset_names, algorithms, perceptor, n_hparams_from, n_hparams, steps,
            ...
            for algorithm in algorithms:
                ...
                for test_envs in all_test_envs:
                    for hparams_seed in range(n_hparams_from, n_hparams):
                        ...
                        train_args['perceptor'] = perceptor # cancle for imagbind
...
if __name__ == "__main__":
    ...
    parser.add_argument('command', choices=['launch', 'delete_incomplete', "del+launch"])
    ...
    command_line_name= f"command_line_{time.time()}.sh"
    for job in jobs:
        print(job)
        # if job.state != Job.DONE:
        #     job.store_command(command_line_name)
