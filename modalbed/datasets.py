...
class ModalityType(StrEnum):
    ...
    POINT = "point"  # for unibind, pointbind


DATASETS = ["MSR_VTT", "VGGSound_S", "NYUDv2"]
...
class MSR_VTT(MultipleModalityDataset):
    ...
    def load_modalities(self):
            ...
            with open(path, "r") as f:
                ...
                for item in data["videos"]:
                    ...
                    video_path = os.path.join(
                        self.path, data_path, item["video_id"] + ".mp4"
                    )
                    audio_path = os.path.join(
                        self.path, "audios", item["video_id"] + ".mp3"
                    )
        ...
        load_(
            os.path.join(
                self.path, "train_val_annotation/train_val_videodatainfo.json"
            ),
            "train_val_videos/TrainValVideo",
        )
        load_(
            os.path.join(self.path, "test_annotation/test_videodatainfo.json"),
            "test_videos",
        )
...
class VGGSound(MultipleModalityDataset):
    ...
    def __init__(self, root, test_envs, hparams):
        ...
        self.path = os.path.join(
            root, "vggsound/scratch/shared/beegfs/hchen/train_data/VGGSound_final/"
        )
    ...
    def extract_audio_from_video(self):
        data_pd = pd.read_csv(
            self.csv_path, names=["youtube_id", "start_sec", "label", "split"]
        )
