...
from .pointbind.models import PointBind_models
from .freebind.models.uni_spaces import InternVL_IB_FT_PP
...
class UniBind(BindModel):
    ...
    def forward(self, inputs):
        ...
        if "point" in inputs:
            ...
            vision_embeddings = self.backbone.bind.modality_postprocessor_point(
                pc_embeddings
            )
...
class ImageBindModal(BindModel):
    ...
    def forward(self, inputs):
        ...
        if modality_value is not None:
            ...
            modality_value = self.model.modality_trunks[self.modal_truck](
                **trunk_inputs
            )
...
class LanguageBind(BindModel):
    ...
    cache_dir = pkg_resources.resource_filename(
        __name__, "languagebind/languagebind/cache_dir"
    )
    image_ckpt_path = pkg_resources.resource_filename(
        __name__, "languagebind/languagebind/ckpts/Image"
    )
    ...
    def __init__(self, pretrain_weights):
        ...
        self.model = languagebind_model(
            clip_type=clip_type, cache_dir=LanguageBind.cache_dir
        )
        ...
        self.tokenizer = LanguageBindImageTokenizer.from_pretrained(
            LanguageBind.image_ckpt_path,
            cache_dir=os.path.join(LanguageBind.cache_dir, "tokenizer_cache_dir"),
        )
...
class PointBind(BindModel):
    embed_dim = 1024

    def __init__(self, pretrain_weights):
        super(PointBind, self).__init__()
        self.model = PointBind_models.PointBind_I2PMAE()
        self.model.load_state_dict(torch.load(pretrain_weights), strict=True)
        self.embed_dim = PointBind.embed_dim
...
class FreeBind(BindModel):
    embed_dim = 1024

    def __init__(self, mode: Literal["Versatile", "AT"] = "AT"):
        super(FreeBind, self).__init__()
        self.model = InternVL_IB_FT_PP(mode=mode)
        self.embed_dim = FreeBind.embed_dim
        self.mode = mode

    def forward(self, inputs: dict):
        from modalbed.datasets import ModalityType as Modality

        embeddings = dict[Modality, torch.Tensor]()
        if Modality.VISION in inputs:
            embeddings[Modality.VISION] = self.model.emb_images(inputs[Modality.VISION])
        if Modality.AUDIO in inputs:
            self.model.factor = 0.8 if "at" in self.mode.lower() else 0.5
            embeddings[Modality.AUDIO] = self.model.emb_audios(inputs[Modality.AUDIO])
        if Modality.TEXT in inputs:
            self.model.factor = 0.5 if "at" in self.mode.lower() else 0.1
            embeddings[Modality.TEXT] = self.model.emb_texts(inputs[Modality.TEXT])
        return embeddings
...
def get_embed_dim(model: str = "imagebind"):
    ...
    elif model == "pointbind":
        return PointBind.embed_dim
    elif model.split("_")[0] == "strongMG":
        return ImageBindModal.embed_dim
    elif model.split("_")[0] == "freebind":
        return FreeBind.embed_dim
    ...
    if model == "imagebind":
        if load_pretrain:
            if load_pretrain_path is None:
                path = pkg_resources.resource_filename(__name__, "imagebind_huge.pth")
            else:
        ...
        if load_pretrain:
            if load_pretrain_path is None:
                path = pkg_resources.resource_filename(__name__, "unibind.pt")
            else:
                path = load_pretrain_path
        model = UniBind(path)
        ...
        if load_pretrain:
            path = load_pretrain_path
        model = ImageBindModal(
            modal_pre,
            modal_truck,
            path,
            pretrain_weights=pkg_resources.resource_filename(
        ...
        if load_pretrain:
            if load_pretrain_path is None:
                path = pkg_resources.resource_filename(__name__, "pointbind_i2pmae.pt")
            else:
                path = load_pretrain_path
        model = PointBind(path)
    elif model.split("_")[0] == "freebind":
        _, mode = model.split("_")
        model = FreeBind(mode=mode)
